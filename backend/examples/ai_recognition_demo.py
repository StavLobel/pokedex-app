...
async def demo_ai_recognition():
    ...
    try:
        ...
        print(f"  Model: {status['model_info']['name']} v{status['model_info']['version']}")
        ...
        if primary.bounding_box:
            ...
            print(f"  Bounding Box: ({bbox.x:.2f}, {bbox.y:.2f}) - {bbox.width:.2f}x{bbox.height:.2f}")
        ...
        if result.alternative_predictions:
            ...
            for i, alt in enumerate(result.alternative_predictions, 1):
                print(f"  {i}. {alt.pokemon_name.title()} (#{alt.pokemon_id}) - {alt.confidence:.1%}")
        
        # Analysis
        print(f"\n📈 Analysis:")
        print(f"  High Confidence: {'Yes' if result.has_high_confidence else 'No'}")
        print(f"  Needs User Selection: {'Yes' if result.needs_user_selection else 'No'}")
        
        # Test multiple images for consistency
        print(f"\n🔄 Testing prediction consistency...")
        result2 = await service.identify_pokemon(image_bytes)
        
        if result.primary_prediction.pokemon_name == result2.primary_prediction.pokemon_name:
        ...
        print(f"  Different Image Result: {result3.primary_prediction.pokemon_name.title()} ({result3.primary_prediction.confidence:.1%})")
...
async def demo_image_preprocessing():
    ...
    try:
        ...
        for name, image, format_type in test_cases:
            ...
            print(f"    Original: {image.size[0]}x{image.size[1]} {image.mode}, {len(image_data):,} bytes")
