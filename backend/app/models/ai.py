...
class Prediction(BaseModel):
    ...
    bounding_box: Optional[BoundingBox] = Field(None, description="Object detection bounding box")
...
class ModelInfo(BaseModel):
    ...
    accuracy: Optional[float] = Field(None, ge=0, le=1, description="Model accuracy on test set")
    input_shape: tuple = Field(..., description="Expected input image shape (height, width, channels)")
...
class ImageProcessingConfig(BaseModel):
    ...
    target_size: tuple = Field(default=(224, 224), description="Target image size (height, width)")
    normalize_mean: List[float] = Field(default=[0.485, 0.456, 0.406], description="Normalization mean values")
    normalize_std: List[float] = Field(default=[0.229, 0.224, 0.225], description="Normalization std values")
