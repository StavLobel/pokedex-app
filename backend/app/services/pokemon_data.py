...
class PokemonDataService:
    ...
    async def cache_pokemon_data(self, pokemon_data: PokemonData) -> None:
        ...
        logger.debug(
            f"Manually cached Pokemon data for {pokemon_data.name} (ID: {pokemon_data.id})"
        )
    ...
    def get_cache_stats(self) -> Dict[str, int]:
        ...
        return {
            ...
            "pokemon_by_id": len(
                [k for k in self._cache.keys() if k.startswith("pokemon_id_")]
            ),
            "pokemon_by_name": len(
                [k for k in self._cache.keys() if k.startswith("pokemon_name_")]
            ),
        }
    ...
    async def _fetch_with_retry(self, url: str) -> Dict[str, Any]:
        ...
        raise PokemonAPIError(
            f"Failed to fetch data from {url} after {self.max_retries + 1} attempts: {last_exception}"
        )
