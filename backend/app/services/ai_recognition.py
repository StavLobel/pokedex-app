...
class PokemonClassifier(ABC):
    ...
    def __init__(self, config: ImageProcessingConfig, confidence_threshold: float = 0.7):
        self.config = config
        ...
        try:
            ...
            if base_confidence < self.confidence_threshold:
                ...
                for _ in range(num_alternatives):
                    ...
                    available_pokemon = [p for p in self.MOCK_POKEMON_DATA if p["id"] not in used_ids]
...
class ImageRecognitionService:
    ...
    async def identify_pokemon(self, image_bytes: bytes) -> PokemonIdentification:
        ...
        try:
            ...
            processing_time_ms = int((time.time() - start_time) * 1000)
            raise PredictionError(f"Pokemon identification failed after {processing_time_ms}ms: {str(e)}")
    ...
    async def validate_image(self, image_bytes: bytes) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if len(image_bytes) > self.config.max_file_size:
                return False, f"Image too large. Maximum size: {self.config.max_file_size / (1024*1024):.1f}MB"
            ...
            if image.format not in self.config.supported_formats:
                return False, f"Unsupported format: {image.format}. Supported: {', '.join(self.config.supported_formats)}"
