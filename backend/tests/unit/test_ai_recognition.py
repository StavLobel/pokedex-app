...
class TestPrediction:
    ...
    def test_prediction_validation(self):
        ...
        prediction = Prediction(pokemon_name="  PIKACHU  ", pokemon_id=25, confidence=0.8)
        assert prediction.pokemon_name == "pikachu"
...
class TestPokemonIdentification:
    ...
    def test_low_confidence_identification(self):
        ...
        assert identification.alternative_predictions[0].confidence >= identification.alternative_predictions[1].confidence
...
class TestMockPokemonClassifier:
    ...
    async def test_prediction_with_loaded_model(self, classifier):
        ...
        await classifier.load_model()
        dummy_image = np.ones((1, 224, 224, 3)) * 0.5  # Consistent input for deterministic results
        ...
        assert predictions[0].pokemon_name in [p["name"] for p in MockPokemonClassifier.MOCK_POKEMON_DATA]
...
class TestImageRecognitionService:
    ...
    @pytest.mark.asyncio
    async def test_identify_pokemon_without_initialization(self, service, sample_image_bytes):
...
async def test_integration_full_pipeline():
    ...
    assert isinstance(result, PokemonIdentification)
    assert result.primary_prediction.pokemon_name in [p["name"] for p in MockPokemonClassifier.MOCK_POKEMON_DATA]
