...
class TestIdentifyPokemonEndpoint:
    ...
    def test_identify_pokemon_success(self, client, valid_image_file_data):
        ...
        response = client.post(
            "/api/v1/identify", files={"image": (filename, file_content, content_type)}
        )
    ...
    def test_identify_pokemon_validation_error(
        self, mock_validate, client, valid_image_file_data
    ):
        ...
        mock_validate.side_effect = ImageValidationError(
    ...
    @patch(
        "app.services.image_validation.image_validation_service.read_and_validate_image"
    )
    def test_identify_pokemon_unexpected_error(
        self, mock_read, client, valid_image_file_data
    ):
    ...
    def test_identify_pokemon_png_file(self, client):
        ...
        response = client.post(
            "/api/v1/identify", files={"image": (filename, img_bytes, "image/png")}
        )
    ...
    def test_identify_pokemon_webp_file(self, client):
        ...
        response = client.post(
            "/api/v1/identify", files={"image": (filename, img_bytes, "image/webp")}
        )
...
class TestErrorResponseFormat:
    ...
    def test_validation_error_response_format(self, client):
        ...
        response = client.post(
            "/api/v1/identify", files={"image": (filename, img_bytes, "image/gif")}
        )
